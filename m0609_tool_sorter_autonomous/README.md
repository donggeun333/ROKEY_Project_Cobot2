...
제어는 `m0609_tool_sorter_unified`에서 재사용하고 이 패키지는 자동운전
정책과 통합용 서비스만 담당합니다.
...
colcon build --symlink-install \
  --packages-select m0609_tool_sorter_autonomous
...
```bash
ros2 launch m0609_tool_sorter_autonomous \
...
```bash
ros2 run m0609_tool_sorter_autonomous autonomous_dashboard
```
...
- 기존 GUI와 기존 `m0609_tool_sorter_unified` launch를 동시에 실행하지
