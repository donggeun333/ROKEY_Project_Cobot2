...
**이 zip에는 `m0609_plug_insert` 패키지만 들어 있다.** 패키지 자체는 완성돼
...
| ② HMI 음성 경로 연결 | `hmi/hmi/app_v5.py` | 중간 |

①만 해도 **액션으로는 바로 쓸 수 있다.** ②는 음성으로 부르고 싶을 때만 한다.

전제: 이 패키지는 `m0609_tool_sorter_unified`에 의존한다 — 13클래스 seg
...
```xml
<depend>m0609_plug_insert</depend>
```

### 확인

```bash
colcon build --symlink-install --packages-select m0609_plug_insert robot_control
...
```bash
ros2 launch m0609_plug_insert plug_insert.launch.py
```
...
일어나지 않는다. `hmi/hmi/revision/app_v4.py`(구버전, 배포 안 됨)에만 발행부가
남아 있던 레거시 경로다.
...
````text
ROS 2 Humble 워크스페이스에서 작업한다. HMI 패키지는 `hmi`, 액션 서버는
`robot_control`, 이 기능의 구현체는 `m0609_plug_insert` 패키지에 있다.

목표: HMI(`hmi/hmi/app_v5.py`)의 음성 명령 `콘센트 체결해줘`(intent
...
검증:
```bash
cd <워크스페이스>/src/cobot2_ws/hmi
python3 -c "
import sys, importlib.util
spec = importlib.util.spec_from_file_location('app_v5', 'hmi/app_v5.py')
...
파이썬 파일만 고쳤으면 재빌드 없이 `ros2 run hmi app_node` 재시작으로 반영된다.
````
