...
``src/cobot2_ws/hmi/pointclouds``를 우선 보고, 없으면 과거 구조인
``src/hmi/pointclouds``를 fallback으로 사용한다.
"""
...
def resolve_hmi_pointcloud_dir(workspace_dir: Path) -> Path:
    """워크스페이스 내 hmi 패키지 위치 변경 전/후를 모두 허용한다."""

    candidates = [
        workspace_dir / "src" / "cobot2_ws" / "hmi" / "pointclouds",
        workspace_dir / "src" / "hmi" / "pointclouds",
    ]
...
def generate_launch_description() -> LaunchDescription:
    ...
    hmi_pointcloud_dir = resolve_hmi_pointcloud_dir(workspace_dir)
    default_bolt_capture_dir = str(hmi_pointcloud_dir / "bolt" / "captures")
    default_outlet_capture_dir = str(hmi_pointcloud_dir / "outlet" / "captures")
