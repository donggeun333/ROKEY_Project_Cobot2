...
> 이 `hmi` 패키지 안으로 병합되었습니다. 현재는 `app_node`(`hmi/app_v5.py`) 하나가
> **Flask HMI + ROS2 브리지 + 음성 인식/안내(`VoiceEngine`)** 를 한 프로세스에서 함께 실행합니다.
> - 음성 처리 모듈: `hmi/hmi/voice/` (`MicController.py`, `wakeup_word.py`, `stt.py`, `tts.py`, `phrases.py`)
> - 통합 로직: `hmi/hmi/app_v5.py` 안의 `VoiceEngine` 클래스 + `AssemblyHmiBridge._on_voice_result()`
> - 기존 개별 노드(Azure STT, `get_keyword_v2` 등)는 `hmi/hmi/legacy/` 에 참고/수동 디버깅용으로만 남겨두었으며,
...
백그라운드 스레드로 구동한다(`hmi/hmi/app_v5.py`). 별도로 실행해야 하는 음성
노드는 없으며, `app_node` 하나만 실행하면 된다.

**동작 흐름**

1. **wake word 상시 감지**: `hmi/hmi/voice/wakeup_word.py`의 `WakeupWord`가 마이크 입력을 계속 읽으며
   `hello_rokey_8332_32.tflite` 모델로 "헬로우 로키"를 감지한다.
2. **감지 시 TTS 안내**: `hmi/hmi/voice/tts.py`(OpenAI TTS)로 "어떤 작업을 하시겠습니까?"를 재생한다.
3. **STT**: `hmi/hmi/voice/stt.py`(OpenAI Whisper)로 5초간 녹음 후 텍스트로 변환한다.
...
`hmi/hmi/legacy/`에 그대로 남아있다. 필요 시 `ros2 run hmi legacy_stt`, `ros2 run hmi legacy_get_keyword`로
