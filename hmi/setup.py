...
# [통합] voice_processing 패키지의 wake word 모델(.tflite)이 hmi/resource 아래로 병합됨
...
setup(
    ...
    data_files=[
        ...
        ("share/" + package_name + "/pointclouds/outlet/captures",
            glob("pointclouds/outlet/captures/*.pcd"),
        ),
        ("share/" + package_name + "/pointclouds/bolt/references",
            glob("pointclouds/bolt/references/*.pcd"),
        ),
        ("share/" + package_name + "/pointclouds/bolt/captures",
            glob("pointclouds/bolt/captures/*.pcd"),
        ),
    ],
    install_requires=[
        'setuptools',
    ...
    extras_require={
        # 레거시(hmi/legacy) 참고용 Azure STT 노드를 계속 쓰고 싶은 경우에만 필요
        'legacy-azure': ['azure-cognitiveservices-speech'],
    },
    zip_safe=True,
    maintainer='rokey',
    maintainer_email='rokey@todo.todo',
    description='CoboTender Assembly/Bartender Robot: Flask HMI + ROS2 bridge + integrated voice pipeline (wake word/STT/LLM/TTS), merged from hmi + voice_processing',
    license='Apache-2.0',
    ...
    entry_points={
        'console_scripts': [
            ...
            'app_node = hmi.app_v5:main',
            'test = hmi.test.test_movej:main',
            # [레거시] 참고/수동 디버깅용 - 기본 실행 흐름에는 포함되지 않음
            'legacy_stt = hmi.legacy.stt_node:main',
            'legacy_get_keyword = hmi.legacy.get_keyword_v2:main',
        ],
    },
)
