...
# ==========================================
# m0609_tool_sorter_handover와 m0609_tool_sorter_autonomous는 각자 진행 상황을
...
# 원본은 m0609_tool_sorter_autonomous의 REQUIRED_TOOL_NAMES(= grasp_selector의
...
class AssemblyHmiBridge(Node):
    ...
    def _handover_status_callback(self, msg):
        """공구 전달(m0609_tool_sorter_handover) 진행 상황 수신 콜백"""
    ...
    def _cleanup_status_callback(self, msg):
        """공구 정리(m0609_tool_sorter_autonomous) 진행 상황 수신 콜백"""
        parsed = parse_tool_status(msg.data)
        if parsed is None:
            return
        with self.lock:
    ...
    def _on_voice_result(self, raw_text):
        ...
        # 공구 가져오기 (TOOL_FETCH) -> m0609_tool_sorter_handover
        ...
        # 공구 정리 (TOOL_CLEANUP) -> m0609_tool_sorter_autonomous
    ...
    def _run_outlet_action(self):
        ...
        실제 인식/파지/삽입은 m0609_plug_insert가 수행한다. 상태 토픽이 없으므로
    ...
    def _run_robot_scan_motion(self, category="outlet", item_index=0):
        ...
        # 워크스페이스의 hmi/pointclouds/<bolt|outlet>/captures/ 에 직접 저장하므로,
