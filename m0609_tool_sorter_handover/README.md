...
좌표변환, 로봇·그리퍼 제어는 `m0609_tool_sorter_unified`에서 재사용하고, 이
패키지는 전달 정책만 담당합니다.
...
colcon build --symlink-install \
  --packages-select m0609_tool_sorter_handover
...
```bash
ros2 launch m0609_tool_sorter_handover handover.launch.py
```
...
- 자동 정리(`m0609_tool_sorter_autonomous`)와 **동시에 실행하지 마십시오.**
